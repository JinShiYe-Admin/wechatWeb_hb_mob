		...
		<script type="text/javascript">
				...
				methods: {
					selectFile: function(event) { //从手机中选择图片后到界面的回调
						...
						for(var i = 0; i < files.length; i++) {
							if(homework.uploadedFiles.length + i > 8) {
								$.alert("最多只能上传9张照片");
								break;
							} else {
								if(event.target.value) {
									qnFileUploader.addFile(files[i], files[i].name);
								}
							}
						}
			...
			function getDepart() {
				...
				unitWebsitePro(tempData, function(data) {
					...
					if(data.RspCode == 0) {
						homework.depart_array = JSON.parse(data.RspData);
			...
			function getPerson(colid, index) {
				if(colid == -1) {
					return;
				}
...
>>>>>>> 03deaa88b130778520e7837f789057a1912f0686
</html>