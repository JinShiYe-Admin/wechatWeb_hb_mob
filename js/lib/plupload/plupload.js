			...
			*/
			var parseUrl = function(url, currentUrl) {
				var key = ['source', 'scheme', 'authority', 'userInfo', 'user', 'pass', 'host', 'port', 'relative', 'path', 'directory', 'file', 'query', 'fragment'],
					...
					regex = /^(?:([^:\/?#]+):)?(?:\/\/()(?:(?:()(?:([^:@\/]*):?([^:@\/]*))?@)?(\[[\da-fA-F:]+\]|[^:\/?#]*)(?::(\d*))?))?()(?:(()(?:(?:[^?#\/]*\/)*)()(?:[^?#]*))(?:\\?([^#]*))?(?:#(.*))?)/,
					m = regex.exec(url || ''),
					isRelative, isSchemeLess = /^\/\/\w/.test(url);
				...
				plupload.extend(this, {
					...
					init: function() {
						...
						initControls.call(self, function(initialized) {
							...
							var queueOpts = plupload.extendImmutable({}, self.getOption(), {
								auto_start: true
							});
		...
		define("moxie/runtime/html5/image/ExifParser", [
			...
			function ExifParser(data) {
				...
				function extractTags(IFD_offset, tags2extract) {
					var data = this;
					var length, i, tag, type, count, size, offset, value, values = [],
						hash = {};
	...
	*/
		...
		*/
		define("moxie/runtime/html4/file/FileInput", [
			...
			function FileInput() {
				var _uid, _mimes = [],
					_options, _browseBtnZIndex; // save original z-index;
